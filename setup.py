...
class World:
    def __init__(self, cell=None, directions=4, filename=None):
        if cell is None:
            cell = Cell
        self.Cell = cell
    ...
    def update(self, thief_win=None, guard_win=None):
        ...
        if thief_win:
            if self.thiefWin != thief_win:
                self.thiefWin = thief_win
                self.history.append(thief_win)
        if guard_win:
            if self.guardWin != guard_win:
                self.guardWin = guard_win
                self.history.append(self.thiefWin)
        self.display.update()
        if len(self.history) > 100:
            self.history.pop(0)
            self.winProb = (self.history[99] - self.history[0])
        self.age += 1
...
class TkinterDisplay:
    ...
    def make_title(self, world):
        text = 'age: %d' % (world.age / 100)
        extra = []
        extra.append('WinProb=%d' % (world.thiefWin / (world.guardWin + world.thiefWin + 1) * 100) + '%')
        extra.append('thiefWin=%d' % world.thiefWin)
        extra.append('guardWin=%d' % world.guardWin)
    ...
    def redraw(self):
        ...
        for row in self.world.grid:
            ...
            f.write(bytes(line.getvalue() * self.size, 'utf-8'))
        f.close()
